...
class Api {
  ...
  // https://api.themoviedb.org/3/movie/popular?api_key=338dbfc18fbe98b709ab0b94f5371b58&language=en-US

  final String _api_key = '338dbfc18fbe98b709ab0b94f5371b58';

  final String _lang = 'en-US';

  Future<List> fetchMovieList(String category) async {
    final uri = Uri.https(
        _url, '/3/movie/$category', {'api_key': _api_key, 'language': _lang});
    final jsonResponse = await _getJson(uri);
    if (jsonResponse == null || jsonResponse[MovieConstants.results] == null) {
      print('Error when fetching data.');
      return null;
    }
    return jsonResponse[MovieConstants.results];
  }
